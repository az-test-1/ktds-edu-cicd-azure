...
Kustomize는 Kubernetes 애플리케이션의 YAML 파일을 템플릿화하지 않고도 구성할 수 있는 도구이다. Kustomize는 오버레이(overlay) 개념을 사용하여 기본 리소스를 수정하고 환경별로 다른 설정을 적용할 수 있다.
...
예제 어플리케이션인 mvp-springboot-jdk17의 manifests 디렉토리 구조이다. 
...
ArgoCD Application에 대한 공통 리소스를 관리한다.
...
ArgoCD Application 배포 후 ArgoCD UI를 사용해 예제 어플리케이션인 mvp-springboot-jdk17을 쿠버네티스에 배포한다.
