...
# acr id 확인
$ az acr show --name $ACR_NAME --resource-group $RG_NAME --query id --output tsv
...
$ az ad sp create-for-rbac \
    --name $SP_NAME \
    --role acrpush \
    --scopes $(az acr show --name $ACR_NAME --resource-group $RG_NAME --query id --output tsv)


{
  "appId": "1ec07496-0b50-4d54-93a2-1acab9789607",
  "displayName": "ktdseduacr-sp",
...
--------------------  ------------------------------------  ------------------------------------  --------------------
http://ktdseduacr-sp  3b943e83-4f1d-45b0-9751-b0f97250b5c9  de3c1c00-a9c1-46a8-93ea-8b897c894ab8  2024-10-01T12:52:42Z
...
$ docker tag ssongman/userlist:v1 ktdseduacr.azurecr.io/ssongman/userlist:v1


# 3) ACR에 Image Upload
$ docker push ktdseduacr.azurecr.io/ssongman/userlist:v1
...
$ az acr repository delete -n ktdseduacr --repository ssongman/userlist -y
...
### (2) **Docker 로그인 (토큰 사용)**
Docker에서 발급받은 토큰을 사용하여 로그인한다. 토큰을 이용한 Docker 로그인은 기본 인증의 **비밀번호 대신 토큰**을 사용한다.

```bash

$ echo <ACCESS_TOKEN> | docker login <ACR_NAME>.azurecr.io --username 00000000-0000-0000-0000-000000000000 --password-stdin
...
# 2) ACR에 Push 하기 위한 tag 변경
$ docker tag ssongman/userlist:v1 ktdseduacr.azurecr.io/$eduusername/userlist:v1


# 3) ACR에 Image Upload
$ docker push ktdseduacr.azurecr.io/$eduusername/userlist:v1
...
$ az acr repository show-tags \
    --name ktdseduacr \
    --repository $eduusername/userlist \
    -o table
...
# 6) ACR에 Upload된 이미지 삭제
$ az acr repository delete -n ktdseduacr --repository $eduusername/userlist -y
...
Admin 계정은 간편하지만, 보안적으로는 더 강력한 **Service Principal**이나 **Azure AD 인증** 방식이 권장된다.

이 방식은 가장 단순하고 전통적인 방식으로, 특히 빠르게 테스트하거나 임시 작업을 수행할 때 유용하다.
