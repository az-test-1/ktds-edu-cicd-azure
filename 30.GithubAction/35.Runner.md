...
Github가 제공하는 기본 호스팅 환경 대신, 사용자가 직접 구성한 환경(서버, VM, 컨테이너 등)에서 workflow를 실행할 수 있도록 설정한 Runner입니다. 사용자는 Runner가 설치된 하드웨어와 소프트웨어에 대해 완전히 제어가 가능하며, 사용자의 요구사항에 맞춰 환경을 구성할 수 있다.
...
## 1) Service로 Self-Hosted Runner Application 구성

셀프 호스팅 러너 애플리케이션을 서비스로 구성하여, 머신이 시작될 때 러너 애플리케이션을 자동으로 시작할 수 있다. (Service daemon과 같이 백그라운드에서 실행되는 프로세스)

Systemd를 사용하는 Linux시스템의 경우, 성공적으로 Self-hosted Runner를 추가한 후 생성되는 [svc.sh](http://svc.sh) 스크립트를 통해 Runner Application을 service로 설치하고 관리할 수 있다. 

Runner Machine에 self-hosted runner 애플리케이션을 설치한 Directory에서 shell을 열고 아래 명령을 사용하여 Service를 설치하고 관리다. 
...
### (1) Actions Runner Controller 구성 요소
...
- CustomResourceDefinition(CRD): ARC에서 사용되는 Runner 관리 리소스는 Kubernetes 기본 리소스가 아닌, CRD를 통해 정의된 사용자 지정 리소스이다. 이 CRD는 'RunnerDeployment', 'AutoscalingRunnerSet'등의 형태로 Runner의 배포와 확장을 관리한다.

### (2) Actions Runner Controller CRD 유형

**1. Autoscaling Runner** 

- 특징 : 자동으로 Runner 수를 조정하는 기능을 제공하며, 트래픽이나 워크플로우 작업 수에 따라 Runner가 자동으로 생성되거나 삭제된다.
- 배포 방법 : AutoscalingRunnerSet 리소스를 사용하여 설정하며, 최소 및 최대 Runner 수, 스케일링 조건을 지정할 수 있다.
...
- 특징 : 각 작업이 완료되면 Runner가 자동으로 삭제되는 임시 Runner이다. 보안 및 리소스 관리 측면에서 유용하다.
- 배포 방법 : EphemeralRunner 리소스를 사용하여 설정하며, 특정 작업을 처리한 후 자동으로 삭제되므로 관리하기 편리하다.
...
1. Repository 내 Actions > workflow 선택 > Run workflow 에서 runner 를 선택합니다.
   ![workflow.png](./assets/workflow-bfc7d2a2-2515-4f5d-9af8-59aa795913d4.png)

2. 선택한 Runner에 따라 Workflow가 실행됩니다.
   ![스크린샷 2024-09-12 오후 3.15.36.png](./assets/스크린샷 2024-09-12 오후 3.15.36-b0b3ad21-f5c5-456a-9db7-ec3097fc7990.png)