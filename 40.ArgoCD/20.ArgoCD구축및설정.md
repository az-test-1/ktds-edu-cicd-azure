...
argocd-applicationset-controller          ClusterIP   192.168.126.96    <none>        7000/TCP,8080/TCP            20s
argocd-dex-server                         ClusterIP   192.168.126.199   <none>        5556/TCP,5557/TCP,5558/TCP   20s
argocd-metrics                            ClusterIP   192.168.126.55    <none>        8082/TCP                     20s
argocd-notifications-controller-metrics   ClusterIP   192.168.126.167   <none>        9001/TCP                     20s
argocd-redis                              ClusterIP   192.168.126.151   <none>        6379/TCP                     20s
argocd-repo-server                        ClusterIP   192.168.126.208   <none>        8081/TCP,8084/TCP            20s
argocd-server                             ClusterIP   192.168.126.128   <none>        80/TCP,443/TCP               20s
argocd-server-metrics                     ClusterIP   192.168.126.31    <none>        8083/TCP                     20s
...
spec:
  ...
  rules:
  - host: argocd.20.249.129.204.nip.io
    http:
      paths:
      - backend:
          service:
...
spec:
  ingressClassName: nginx
  rules:
  - host: argocd.4.230.144.108.nip.io
    http:
      paths:
      - backend:
          service:
...
# 확인
$ curl https://argocd.20.249.129.204.nip.io/ -i -k
HTTP/2 200


$ curl http://argocd.20.249.129.204.nip.io/ -i
HTTP/1.1 200 OK
...
$ argocd account update-password \
    --account admin \
    --current-password NVZEShpu5rIwHPxp \
    --new-password Prd1234! \
    --grpc-web

Password updated
Context 'argocd.4.230.144.108.nip.io' updated
...
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


# class token( ~ 2023/9/17)
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    <-- 이걸로 하니깐 잘되네...

# 위 토큰으로 아래처럼 clone시에도 사용가능하다.
$ git clone https://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@github.com/ssongman/argocd.git
...
```sh
# 기본서버 주소 설정
export ARGOCD_SERVER=https://argocd.20.249.129.204.nip.io/
...
# 확인 - 서버 지정
$ argocd app list \
    --server argocd.20.249.129.204.nip.io
...
# 기본서버 주소 설정
export ARGOCD_SERVER=argocd.20.249.129.204.nip.io
